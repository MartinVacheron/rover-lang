...
pub const AnalyzerMsg = union(enum) {
    ...
    already_declared_in_trait: struct { trait: []const u8, name: []const u8 },
    already_impl_trait: struct { ty: []const u8, trait: []const u8 },
    ...
    enum_discr_duplicate: struct { value: i64 },
    enum_discr_not_comptime,
    enum_discr_not_int: struct { found: []const u8 },
    error_not_in_union: struct { found: []const u8, expect: []const u8 },
    err_union_err_lhs: struct { found: []const u8 },
    err_union_non_err_rhs: struct { found: []const u8 },
    ...
    missing_fn_in_trait: struct { func: []const u8, trait: []const u8 },
    ...
    op_overload_arg_count: struct { expect: usize, found: usize },
    ...
    return_outside_fn,
    self_outside_decl,
    shift_overflow: struct { max: usize, found: i64 },
    ...
    undeclared_field: struct { structure: []const u8, field: []const u8 },
    ...
    union_constr_expect_one_arg: struct { got: usize },
    union_constr_useless_paren,
    union_constr_named_arg,
    ...
    pub fn getMsg(self: Self, writer: *Writer) !void {
        try switch (self) {
            ...
            .already_declared_in_trait => |e| writer.print("function '{s}' is already declared in trait '{s}'", .{ e.name, e.trait }),
            .already_impl_trait => |e| writer.print("trait '{s}' is already implemented for type '{s}'", .{ e.trait, e.ty }),
            ...
            .div_by_zero => writer.writeAll("illegal division by zero"),
            ...
            .enum_discr_not_comptime => writer.writeAll("enum's discriminants must be constants known at compile time"),
            .enum_discr_not_int => |e| writer.print("enum discriminant values must be of type 'int', found '{s}'", .{e.found}),
            .error_not_in_union => |e| writer.print("error '{s}' is not part of union '{s}'", .{ e.found, e.expect }),
            .err_union_err_lhs => |e| writer.print("expect a non error type on left side of '!' in error unions, found: '{s}'", .{e.found}),
            .err_union_non_err_rhs => |e| writer.print("expect an error type on right side of '!' in error unions, found: '{s}'", .{e.found}),
            ...
            .match_regular_on_union => |e| writer.print("must use 'match is' when matching unions, found: '{s}'", .{e.found}),
            .match_useless => writer.writeAll("pattern matching with only wildcard can be written as a single expression"),
            .match_unreachable_arm => writer.writeAll("unreachable arm"),
            .match_useless_alias => writer.writeAll("no need to create an alias for an identifier"),
            ...
            .missing_fn_in_trait => |e| writer.print("function '{s}' is not part of trait '{s}' definition", .{ e.func, e.trait }),
            .missing_fn_impl_in_trait => |e| writer.print("missing function '{s}' implementation for trait '{s}'", .{ e.func, e.trait }),
            ...
            .op_overload_arg_count => |e| writer.print("expect {} arguments to overload operator, found {}", .{ e.expect, e.found }),
            .op_overload_void_return => writer.writeAll("operator overload has to retrun a non-void type"),
            ...
            .shift_overflow => |e| writer.print("shift overflow, max possible shift is {}, found {}", .{ e.max, e.found }),
            ...
            .trait_default_value => writer.writeAll("can't declare default value for trait functions"),
            ...
            .undeclared_field => |e| writer.print("Type '{s}' doesn't have field '{s}'", .{ e.structure, e.field }),
    ...
    pub fn getHelp(self: Self, writer: *Writer) !void {
        try switch (self) {
            ...
            .div_by_zero => writer.writeAll("dividing by zero is a math error, try protecting division with an epsilon value (1e-6)"),
            ...
            .enum_discr_not_int => writer.writeAll("discriminant values must be integers (negative or positive)"),
            ...
            .match_useless_alias => writer.writeAll(
                \\creating aliases with '@<name>' is usefull in cases where the value being match on is a temporary one
                \\like matching a function's return value. When using an identifier, you can use it directly
                \\When using 'match is' with an identifier, its type will be infered based on the matched arm
            ),
            ...
            => writer.writeAll("refer to trait's definition to see which functions have to be implemented"),
            ...
            .op_overload_arg_count => writer.writeAll("expect 2 arguments when overloading binary operators and only 1 for unary"),
            .op_overload_void_return => writer.writeAll("overloaded operators are called in expressions, they have to produce a value"),
            ...
            .shift_overflow => writer.writeAll("reduce shift amount"),
            ...
            .trait_default_value => writer.writeAll("trait functions can't have default values but implementations of the functions can"),
            ...
            .undeclared_field => writer.writeAll("refer to structure's definition to see available fields or modify it"),
