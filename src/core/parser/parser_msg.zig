...
pub const ParserMsg = union(enum) {
    ...
    default_value_self,
    empty_error_set,
    implicit_select_non_ident,
    err_type_not_ident,
    expect_brace_end_container: struct { kind: []const u8 },
    expect_brace_after: struct { what: []const u8 },
    ...
    pub fn getMsg(self: Self, writer: *Writer) !void {
        try switch (self) {
            ...
            .implicit_select_non_ident => writer.writeAll("expect an identifier for implicit selector syntax"),
            ...
            .missing_paren_array_type => writer.writeAll("missing closing parenthesis for array's child type"),
    ...
    pub fn getHelp(self: Self, writer: *Writer) !void {
        try switch (self) {
            ...
            .implicit_select_non_ident => writer.writeAll("implicit selector syntax is: '.tag_name' for enums and '.tag_name(value...)' for unions"),
            ...
            .expect_brace_before => writer.writeAll("add an opening brace '{'"),
            ...
            .missing_array_close_bracket => writer.writeAll("syntax to decalre array is: 'var array = [<value>, <value>, ...]'"),
            .missing_array_access_close_bracket => writer.writeAll("syntax to index array is: 'array[<index>]'"),
            ...
            .missing_paren_array_type => writer.writeAll("when using parenthesis for array's child type, it must be closed"),
    ...
    pub fn expectBraceAfter(name: []const u8) Self {
        return .{ .expect_brace_after = .{ .what = name } };
    }

    pub fn expectBraceBefore(name: []const u8) Self {
        return .{ .expect_brace_before = .{ .what = name } };
    }

    pub fn expectName(name: []const u8) Self {
        return .{ .expect_name = .{ .name = name } };
    }
};
